...
function convertToDateObj(dateValue) {
    let
        day = Number(dateValue.substr(8, 2)),
        month = Number(dateValue.substr(5, 2)),
        year = Number(dateValue.substr(0, 4));

    dateValue = new Date(year,month,day);
    //2015-09-08 / date = new Date(year,month,day);
    dateValue = Date.UTC(dateValue.getFullYear(),dateValue.getMonth(),dateValue.getTime());
...
function cometSearch (){
    ...
    let
        dayStart = Number(START_DATE.substr(8, 2)),
        monthStart = Number(START_DATE.substr(5, 2)),
        yearStart = Number(START_DATE.substr(0, 4)),

        dayEnd = Number(START_DATE.substr(8, 2)),
        monthEnd = Number(START_DATE.substr(5, 2)),
        yearEnd = Number(START_DATE.substr(0, 4));
...
// loop for every day
    ...
    for (counter; counter < 7; counter++){
        ...
        let
            id = String(Object.values(comets)[2][jsonDate][0]["id"]),
            name = String((Object.values(comets)[2][jsonDate][0]["name"])
                .replace("(","").replace(")","")),
            absoluteMagnitude = String(Object.values(comets)[2][jsonDate][0]["absolute_magnitude_h"]),
            estimatedDiameterAvg = String((((
                Object.values(comets)[2][jsonDate][0]["estimated_diameter"]["meters"]["estimated_diameter_max"]
                + Object.values(comets)[2][jsonDate][0]["estimated_diameter"]["meters"]["estimated_diameter_min"])/2)).toFixed(2)),
            potentiallyHazardous = Comet.hazardLevel(Object.values(comets)[2][jsonDate][0]["is_potentially_hazardous_asteroid"]),
            closeApproach = String(Object.values(comets)[2][jsonDate][0]["close_approach_data"][0]["close_approach_date_full"])
                .replace(" ", " - "),
            relativeVelocity = String(Number(
                Object.values(comets)[2][jsonDate][0]["close_approach_data"][0]["relative_velocity"]["kilometers_per_hour"]).toFixed(2)),
            missDistance = String(Number(Object.values(comets)[2][jsonDate][0]["close_approach_data"][0]["miss_distance"]["kilometers"])
                .toFixed(2)),
            sentryObject = Comet.hitChanceNext100Years(String(Object.values(comets)[2][jsonDate][0]["sentry_object"]));
